...
// The summary line's book title in the active locale - the cart embeds
// *items.product, so the Vietnamese title resolves through the same helper the
// listing, PDP and cart use; the core English fields are the fallback.
function lineTitle(
...
export function CheckoutForm() {
  ...
  const inputClass =
    "w-full rounded-[var(--radius-button)] border border-[var(--hairline)] bg-[var(--paper)] px-4 py-2.5 text-sm text-[var(--label-primary)] placeholder:text-[var(--label-tertiary)] transition-colors duration-200 focus:border-[var(--accent)]";
      ...
      <div className="flex flex-col gap-8">
        ...
        <fieldset>
          ...
          <div className="flex flex-col gap-3">
            <label
              className={`flex items-center gap-3 rounded-[var(--radius-button)] border px-4 py-3 text-sm ${
                method === "cod"
                  ...
                  : "border-[var(--hairline)] bg-[var(--paper)]"
            ...
            <label className="flex cursor-not-allowed items-center gap-3 rounded-[var(--radius-button)] border border-[var(--hairline)] bg-[var(--paper)] px-4 py-3 text-sm opacity-60">
              <input
                type="radio"
                name="payment"
                checked={method === "card"}
          ...
          {items.map((item) => (
            ...
            >
              <span className="min-w-0 truncate text-[var(--label-secondary)]">
                {item.quantity}x {lineTitle(item, locale)}
              </span>
